...
then
  ...
  then
    git tag -f $version
    echo "You should override the remote tag too: git push origin $version --no-verify --force";echo
  fi
else
  echo "connextproject/$image:$version does not exist on docker hub yet"
  git tag $version
  echo "You should share this tag: git push origin $version --no-verify";echo
fi
